...
def _get_target_season() -> int:
    ...
    # 決勝レース時間の取得
    list_race_time_1 = [x if x is not None else None for x in ergast_1.get_event_time(event="race")]
    list_race_time_2 = [x if x is not None else None for x in ergast_2.get_event_time(event="race")]
